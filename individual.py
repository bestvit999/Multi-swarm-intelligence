...
class Individual:
    def __init__(self):
        ...
        self.__vr = np.zeros(cf.get_dimension())
    ...
    def get_cuckoo_adaptive(self):

        step_size = cf.get_stepsize() * levy_flight(2.0) #levy_flight(cf.get_lambda())

        # Update adaptive learning rate
        beta = 0.9
        self.__vr = beta * self.__vr + (1 - beta) * np.power(step_size, 2)
        vr_sqrt = np.sqrt(self.__vr)

        # Update position
        self.__position = self.__position + step_size / (vr_sqrt + 1e-07)
    ...
    def get_cuckoo_dynamic_adaptive(self, current_iter):

        step_size = cf.get_dynamic_step_size(current_iter) * levy_flight(cf.get_dynamic_lambda(current_iter))
